...
class XMLStorage extends PCStorage
{
    ...
    protected function checkURI($xmlFile)
    {
        ...
        if (!is_writable($xmlFile)) {
            throw new Billmate_FileNotWritableException($xmlFile);
        }

        if (!is_readable($xmlFile)) {
            throw new Billmate_FileNotReadableException($xmlFile);
        }
    }
    ...
    public function load($uri)
    {
        ...
        if (!@$this->dom->load($uri)) {
            throw new Billmate_XMLParseException($uri);
        }
    ...
    public function save($uri)
    {
        ...
        if (!$this->dom->save($uri)) {
            throw new BillmateException('Failed to save XML document!');
        }
    }
