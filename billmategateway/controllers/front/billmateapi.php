...
class BillmategatewayBillmateapiModuleFrontController extends ModuleFrontController {
	...
	public function checkAddress()
	{
		...
		$fullname = $shipping->firstname.' '.$shipping->lastname.' '.$shipping->company;

		if (isset($address['company']))
			$apiName = $address['firstname'].' '.$address['lastname'].' '.$address['company'];
		else
	...
	public function prepareDirect($method)
	{
		...
		$paymentData['Card'] = array(
			'promptname' => ($method == 'cardpay' && Configuration('BILLMATE_CARD_PROMPT')) ? 1 : 0,
			'3dsecure' => ($method == 'cardpay' && Configuration('BILLMATE_CARD_3DSECURE')) ? 1 : 0,
