    ...
    {foreach $settings as $setting }
        ...
        {if $setting.type == 'multiselect'}
            <div class="input-row" {if isset($setting.id)} id="{$setting.id}"{/if}>
                <span>{$setting.label}</span>
                <select multiple="multiple" {if $setting.name == 'activateStatuses' && $activation_status == 0}style="display: none;{/if} {if isset($setting.id)}id="{$setting.id}"{/if} {if isset($setting.name)}name="{$setting.name}"{/if}>
                    {html_options options=$setting.options selected=$setting.value}
                </select>
            </div>
        {/if}
