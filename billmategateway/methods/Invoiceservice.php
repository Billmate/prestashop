 ...
 * Time: 18:12
 */
class Invoiceservice extends BillmateGateway {
    ...
    public function __construct()
    {
        ...
        $this->displayName          = $this->module->l('Billmate Invoiceservice','invoiceservice');
        ...
        $this->sort_order           = (Configuration::get('BINVOICESERVICE_SORTORDER')) ? Configuration::get('BINVOICESERVICE_SORTORDER') : 1;
        $this->limited_countries    = array('se');
        $this->allowed_currencies   = array('SEK','EUR','DKK','NOK','GBP','USD');
        $this->authorization_method = false;
        $this->validation_controller = $this->context->link->getModuleLink('billmategateway', 'billmateapi', array('method' => 'invoiceservice'));
    ...
    {
        if (Configuration::get('BINVOICESERVICE_ENABLED') == 0)
            return false;
    ...
    public function getSettings()
    {
        ...
        $settings['minimum_value'] = array(
            'name'     => 'invoiceserviceBillmateMinimumValue',
            'required' => false,
        ...
        $settings['maximum_value'] = array(
            'name'     => 'invoiceserviceBillmateMaximumValue',
            'required' => false,
        ...
        $settings['sort'] = array(
            'name'     => 'invoiceserviceBillmateSortOrder',
            'required' => false,
    ...
    {
        $feeArray        = array();
        $feeArray['fee'] = (Configuration::get('BINVOICESERVICE_FEE') > 0) ? Configuration::get('BINVOICESERVICE_FEE') : 0;
        if ($feeArray['fee'] > 0)
        {
            $tax           = new Tax(Configuration::get('BINVOICESERVICE_FEE_TAX'));
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
        {
            ...
            foreach ($package_list as $id_address => $packageByAddress)
                foreach ($packageByAddress as $id_package => $package)
                {
                    ...
                    $total_fee = 0;
                    if($deliveries == 0) {
                        $billmate_invoice_fee = Configuration::get('BINVOICESERVICE_FEE');
